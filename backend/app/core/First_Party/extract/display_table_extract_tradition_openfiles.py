...
def table_handle_no_rowspan(table):
    ...
    for i in range(0, len(rows)):
        ...
        if th_list and 'colspan' not in th_list and 'false_colspan' not in th_list:
            for th in th_list:
                sdk_dict[th] = None
...
if __name__ == '__main__':
    ...
    #单个隐私政策提取sdk调试
    html_folder_path = r"D:\DingDing\files\PrivacyLegal"
    excel_path = r"D:\DingDing\files\PrivacyLegal\apps_info.xlsx"
    df = pd.read_excel(excel_path)
    html_name_list = df.iloc[:, 3]
    ...
    for filename in os.listdir(html_folder_path):
        if filename.endswith('.html'):
            ...
            if count>start_count and count<=end_count:
                ...
                matching_rows = df[html_name_list == filename].index
                if not matching_rows.empty:
                    package_name = df.iat[matching_rows[0], 2]
                    json_name = package_name + '_sdk.json'

                json_path = os.path.join('temp_sdk_result',json_name)

                if not os.path.exists('\\temp_sdk_result'):
                    # 如果文件夹不存在，则创建该文件夹
                    os.mkdir('\\temp_sdk_result')
                ...
                # #隐私政策sdk子链接提取：
                # run_sdk_link_judge(url = None, soup=None)